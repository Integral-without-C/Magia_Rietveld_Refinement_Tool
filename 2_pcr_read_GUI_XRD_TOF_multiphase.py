...
def detect_and_convert_to_utf8(filepath, encodings=('utf-8', 'gbk', 'gb2312', 'latin1')):
    """
    检测文件编码，如果不是utf-8则转换为utf-8，返回utf-8文件路径
    """
    last_exc = None
...
def ensure_chi2_line(filepath):
    """
    检查并补全 ! Current global Chi2 (Bragg contrib.) = 行
    """
...
def parse_xrd_pcr(filepath, atom_names):
    """
    解析XRD类型pcr文件，返回参数列表
    """
    with open(filepath, 'r', encoding='utf-8') as f:
        ...
        if atom_start > 0:
            for atom in atom_names:
                for idx2 in range(atom_start, len(phase_lines)):
                    if phase_lines[idx2].strip().startswith(atom + " "):    #彻底解决原子名部分重复导致的重复识别问题
                        # 提取X,Y,Z,Biso,Occ
...
def parse_tof_pcr(filepath, n_bg, atom_names):
    with open(filepath, 'r', encoding='utf-8') as f:
        lines = f.readlines()
    params = []
    param_id = 1
    ...
    for pidx in range(len(phase_indices)-1):
        ...
        for idx, l in enumerate(phase_lines):
            if '!Absorption correction parameters' in l:
                # phase_lines[idx+1] 是参数行
                vals = phase_lines[idx+1].split()
                # 取第二和第四个数（下标1和3）
                params.append({
                    "id": param_id,
                    "name": "Abs1",
                    "line": phase_start + idx + 2,  # 全局行号
                    "position": 1,
                    "phase": phase_no,
                    "group": "吸收矫正参数"
                }); param_id += 1
...
def parse_pcr_auto(filepath, n_bg, atom_names):
    """
    自动判断类型并调用对应解析函数
    """
    ...
    if job_type == 0:
        return parse_xrd_pcr(filepath, atom_names)
    elif job_type == -1:
        return parse_tof_pcr(filepath, n_bg, atom_names)
    else:
...
class ParamLibGUI(QWidget):
    def __init__(self):
        ...
        self.atom_edit = QLineEdit("输入所有原子名称（确保pcr中没有重名！）")
    ...
    def on_recognize(self):
        ...
        try:
            # 这里不再需要再次检测编码，直接用self.pcr_path
            self.params = parse_pcr_auto(self.pcr_path, 0, atom_names)
            self.refresh_tabs()
        except Exception as e:
